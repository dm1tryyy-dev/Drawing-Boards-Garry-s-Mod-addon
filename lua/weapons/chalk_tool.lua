...
function SWEP:Initialize()
    ...
    if CLIENT then
        ...
        self.CurrentSizeValue = ChalkMarkerConfig.GetSizeValue("chalk", "draw", "medium")
        self.CurrentEraseSize = "medium"
        self.CurrentEraseSizeValue = ChalkMarkerConfig.GetSizeValue("chalk", "erase", "medium")
...
function SWEP:GetDrawSizeValue()
    ...
    return ChalkMarkerConfig.GetSizeValue("chalk", "draw", sizeName)
end

function SWEP:GetEraseSizeValue()
    local sizeName = self:GetPlayerEraseSize()
    return ChalkMarkerConfig.GetSizeValue("chalk", "erase", sizeName)
end
...
if CLIENT then
    ...
    -- Сбрасываем состояние при смене оружия
    ...
    -- Сбрасываем состояние при смерти или изменении владельца
    function SWEP:OwnerChanged()
        hintState.showTime = 0
        hintState.fadingOut = false
        hintState.alpha = 0
